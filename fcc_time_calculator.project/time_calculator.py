...
def build_time_structure(hours, minutes, days, day_int, display_weekday=False):
    day_int = days+day_int
    total_time = (0000,0,0,hours,minutes,00,day_int,0,00)
    if display_weekday == False:
        time_formatted = strftime("%I:%M %p", total_time ).lstrip("0")
    elif display_weekday == True:
        time_formatted = strftime("%I:%M %p, %A", total_time ).lstrip("0")
    return days, time_formatted
